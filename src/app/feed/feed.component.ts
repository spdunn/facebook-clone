...
export class FeedComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.feedSub = this.store.select('feed').subscribe(
      posts => {
        this.posts = posts.posts;
        // Only get posts from yourself or friends
        this.myPosts = posts.posts.filter(p => {
          return this.currentUser.friends.find(f => f.id == p.author.id) || p.author.id == this.currentUser.id;
        })
      }
    )
  }
  ...
  onPost(form: NgForm) {
    console.log(form)
    let newPost: Post = new Post((this.posts.length + 1).toString(), this.currentUser, form.value.message, [this.currentUser]);
    this.store.dispatch(new AddPost(newPost));
    form.reset();
  }

  onLike(post: Post) {
    var likedPost = new Post(post.id, post.author, post.content, [...post.likes, this.currentUser])
  ...
  onShare(post: Post) {
    ...
    var sharedPost = new Post((this.posts.length + 1).toString(), newAuthor, post.content, [this.currentUser]);
